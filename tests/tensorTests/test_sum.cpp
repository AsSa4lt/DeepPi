...
// Test 2 - Adding Tensors with Equal Sizes
TEST(TensorSumTest, EqualSizes) {
    std::array<uint32_t, 2> dims = {3, 3};
    ...
    EXPECT_FLOAT_EQ(C(0, 1), 10.0f);
    EXPECT_FLOAT_EQ(C(0, 2), 10.0f);
    EXPECT_FLOAT_EQ(C(1, 0), 10.0f);
    EXPECT_FLOAT_EQ(C(1, 1), 10.0f);
    EXPECT_FLOAT_EQ(C(1, 2), 10.0f);
    EXPECT_FLOAT_EQ(C(2, 0), 10.0f);
    EXPECT_FLOAT_EQ(C(2, 1), 10.0f);
    EXPECT_FLOAT_EQ(C(2, 2), 10.0f);
}

// Test 3 - Adding Tensors with Different Sizes (Expecting Failure)
TEST(TensorSumTest, DifferentSizes) {
...
// Test 4 - Edge Case: Zero-Sized Tensors
TEST(TensorSumTest, ZeroSizedTensor) {
...
// Test 5 - Performance
TEST(TensorSumTest, PerformanceTest) {
